...
/**
 * Implementation of {@link RetsHttpResponse} that wraps an Apache {@link CloseableHttpResponse}.
 *
 * <p>It captures response headers, cookies, and the raw response body so callers can repeatedly
 * access the response body via {@link #getInputStream()}. Header names are stored case-insensitively.
 *
 * <p>Instances must be closed via {@link #close()} to release underlying resources.
 */
public class RetsHttpResponseImpl implements RetsHttpResponse, AutoCloseable {
    ...
    /**
     * Constructs a response wrapper and reads the response body into memory.
     *
     * @param response the underlying HTTP response; must not be null
     * @param cookies map to populate with cookies parsed from Set-Cookie headers
    ...
    /**
     * Parses a Set-Cookie header robustly, preserving quoted values that may contain semicolons.
     *
     * @param headerValue the raw Set-Cookie header value
     * @return a map of cookie names to their values (case-insensitive keys)
     */
    ...
    /**
     * Returns the HTTP response status code from the underlying response.
     *
    ...
    /**
     * Returns the value of the specified cookie parsed from Set-Cookie headers.
     *
    ...
    /**
     * Extracts the charset parameter from the Content-Type header, if present.
     *
     * @return the charset value or {@code null} if not present
     */
    ...
    /**
     * Returns a new InputStream over the cached raw response bytes. Callers may read this stream
     * multiple times to access the response body.
     *
     * @return an InputStream positioned at the start of the response body
     */
    @Override
    ...
    /**
     * Closes the underlying HTTP response and releases resources.
     *
     * @throws IOException if closing the underlying response fails
     */
    @Override
